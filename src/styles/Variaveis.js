export const marginConteudoLaterais = '10%'

export const fundoPrincipal = "#FFFFFF";
export const fundoSecundario = "#F5F5F5";
export const HeaderFooter = "linear-gradient(90deg, rgba(206,148,50,1) 0%, rgba(177,108,4,0.9724090319721639) 63%, rgba(206,148,50,1) 96%)";

export const textoPrimario = "#331900";
export const textoFocoHover = "#614b44";
export const textoClicavel = "#272a20";






export const headerFooter = "#D7BD9D";
export const textoBotao = '#FFF'
export const botaoNormal = "#ce9432";
export const botaoHover = "#D9A788";
export const botaoFoco = "#C28A6E";
export const bordaLinha = "#D6C3A5";
export const fundoInput = "#FFF8E6";
export const placeholderInput = "#A69888";
export const destaqueSucesso = "#8FB580";
export const destaqueErro = "#E56B6F";
